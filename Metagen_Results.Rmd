...
phy_obj <- merge_phyloseq(phy_obj_F_Soil,phy_obj_F_Root)

phy_norm <- merge_phyloseq(phy_norm_F_Soil,phy_norm_F_Root)

phy_raref <- merge_phyloseq(phy_raref_F_Soil,phy_raref_F_Root)

ord <- ordinate(phy_raref, method = "PCoA", distance = "bray", weighted=FALSE)

plot_ordination(phy_norm, ord, color = "Soil", shape = "sample_site") + geom_point(size=7) +
...
plot1<- gg_ordiplot(ord$vectors, groups = sample_data(phy_norm)$sample_site, hull = TRUE, 
            label = TRUE, spiders = TRUE, ellipse = FALSE, pt.size = 7, plot = FALSE)
...
Root_Pc_V <- t(otu_table(phy_norm_FR_mrg))[colnames(otu_table(phy_norm_FR_mrg)) %in% common_V]

Root_Pt_V <- t(otu_table(phy_norm_FR_mrg))[colnames(otu_table(phy_norm_FR_mrg)) %in% common_V]
