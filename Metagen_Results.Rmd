...
knitr::opts_knit$set(root.dir="/Users/davidcastro/Desktop/PhD/Projects/7_Chile")
```
...
library(tidyverse) #co-ocurrence network
library(corrr) #co-ocurrence network
library(igraph) #co-ocurrence network
library(ggraph) #co-ocurrence network
```

#Survival
```{r}
survival <- read.csv("survival_rate_2019.csv", sep = ";")
...
```{r}
# Kaplan-Meier curve
surv_object <- Surv(time=survival$Time, survival$Survival)
surv_object

fit1 <- survfit(surv_object ~ Specie+Soil, data= survival)
summary(fit1)

plot1 <- ggsurvplot(fit1, data=survival, pval=TRUE, risk.table = TRUE,
           tables.height = 0.2, tables.theme = theme_cleantable())
curv_facet <- plot1$plot + scale_color_manual(values = c("darkgoldenrod3","brown",  "gold", "darkorange", "darkgoldenrod3","brown",  "gold", "darkorange")) + scale_fill_manual(values = c("darkgoldenrod3","brown",  "gold", "darkorange","darkgoldenrod3","brown",  "gold", "darkorange")) + 
  facet_grid(. ~ Specie)
...
tbl_facet <- plot1$table + facet_grid(.~ Specie, scales = "free")
...
#pdf("Manuscript/Plots/Survival.pdf", width = 19, height = 19)
grid::grid.draw(g)
#dev.off()
```
...
fit.coxph <- coxph(surv_object ~ Specie+Soil, data = survival)

#pdf("Manuscript/Plots/cox_hazard.pdf", width = 19, height = 19)
ggforest(fit.coxph, data = survival) 
#dev.off()
```
##P.chilensis
```{r}
survival_chil <- survival %>% filter(Specie == "P. chilensis")

# Kaplan-Meier curve
surv_object <- Surv(time=survival_chil$Time, survival_chil$Survival)
surv_object

fit1 <- survfit(surv_object ~Soil, data= survival_chil)
summary(fit1)

plot1 <- ggsurvplot(fit1, data=survival_chil, pval=TRUE, risk.table = TRUE,
           tables.height = 0.2, tables.theme = theme_cleantable())
curv_facet <- plot1$plot + scale_color_manual(values = c("darkgoldenrod3","brown",  "gold", "darkorange")) + scale_fill_manual(values = c("darkgoldenrod3","brown",  "gold", "darkorange"))

curv_facet

# Generate risk table for each facet columns
tbl_facet <- plot1$table
tbl_facet + theme(legend.position = "none")
...
#pdf("Manuscript/Plots/survival_chil.pdf", width = 19, height = 19)
grid::grid.draw(g)
#dev.off()

#Cox proportional hazards
fit.coxph <- coxph(surv_object ~ Soil, data = survival_chil)

#pdf("Manuscript/Plots/cox_hazard_chil.pdf", width = 19, height = 19)
ggforest(fit.coxph, data = survival_chil) 
#dev.off()
```
### Without Pozo Almonte soil
```{r}
survival_chil <- survival_chil %>% filter(Soil != "North")
...
plot1 <- ggsurvplot(fit1, data=survival_chil, pval=TRUE, risk.table = TRUE,
           tables.height = 0.2, tables.theme = theme_cleantable())
curv_facet <- plot1$plot + scale_color_manual(values = c("darkgoldenrod3",  "gold", "darkorange")) + scale_fill_manual(values = c("darkgoldenrod3",  "gold", "darkorange"))
...
##P.tamarugo
```{r}
survival_tam <- survival %>% filter(Specie == "P.tamarugo")

# Kaplan-Meier curve
surv_object <- Surv(time=survival_tam$Time, survival_tam$Survival)
surv_object

fit1 <- survfit(surv_object ~ Soil, data= survival_tam)
summary(fit1)

plot1 <- ggsurvplot(fit1, data=survival_tam, pval=TRUE, risk.table = TRUE,
           tables.height = 0.2, tables.theme = theme_cleantable())
curv_facet <- plot1$plot + scale_color_manual(values = c("darkgoldenrod3", "brown", "gold", "darkorange")) + scale_fill_manual(values = c("darkgoldenrod3", "brown", "gold", "darkorange"))
...
#pdf("Manuscript/Plots/survival_tam.pdf", width = 19, height = 19)
grid::grid.draw(g)
#dev.off()

#Cox proportional hazards
fit.coxph <- coxph(surv_object ~ Soil, data = survival_tam)

#pdf("Manuscript/Plots/cox_hazard_tam.pdf", width = 19, height = 19)
ggforest(fit.coxph, data = survival_tam) 
#dev.off()
```
### Without Pozo Almonte soil
```{r}
survival_tam <- survival_tam %>% filter(Soil != "North")
...
# Growth data
```{r}
crecimiento <- read.csv("growth_data.csv", dec = ",", sep = ";")

crecimiento$Specie = factor(crecimiento$Specie, levels=c('P. chilensis','P. tamarugo'))
crecimiento$Period = factor(crecimiento$Period, levels=c('Growth','Drought','Recovery'))
crecimiento$Treatment = factor(crecimiento$Treatment, levels=c('Control','Stress'))
crecimiento$Soil <- rep(c('Sand' = 'La Serena',
                          'Central' = 'Vicuña',
                          'South' = 'Los Angeles'),each = 5)
crecimiento$Soil = factor(crecimiento$Soil, levels=c('La Serena','Vicuña','Los Angeles'))
```

```{r echo=FALSE}
ggplot(crecimiento, aes(x=Period, y=Height, colour=Treatment)) + 
  geom_boxplot() + 
  scale_colour_manual(values = c("darkgreen","darkred")) + 
  facet_grid(Soil~Specie)+
  ggtitle("Total growth per period") +
  scale_y_continuous(name = "Height (cm)", limits = c(0,45))+
  theme_bw()+
  theme(plot.title=element_text(size=rel(1.5), face="bold", colour="Black", hjust = .5),
        axis.text = element_text(size=rel(1)),
        strip.text.x = element_text(size=rel(1.2), face = "italic"),
...
```{r}
ggplot(crecimiento, aes(x=Soil, y=Height, fill=Period)) + 
  geom_boxplot() + 
  scale_fill_manual(values = c("green","green3","green4")) + 
  facet_grid(Specie~.)+
  ggtitle("Total growth per period") +
  ...
  theme(plot.title=element_text(size=rel(1.5), face="bold", colour="Black", hjust = .5),
        axis.text = element_text(size=rel(1)),
        strip.text.y = element_text(size=rel(1.2),face = "italic"),
        axis.title.y = element_text(size = rel(1.1)),
        axis.title.x = element_blank(),
        legend.text = element_text(size = rel(1)),
        legend.title = element_text(size = rel(1.1)))
...
```{r}
#pdf("Manuscript/Plots/growth.pdf", width = 19, height = 19)
ggplot(crecimiento, aes(x=Specie, y=Height, fill=Soil)) +
  stat_boxplot(geom = "errorbar") +
  geom_boxplot(size = 1) + 
  scale_fill_manual(values = c("gold","darkgoldenrod3","darkorange")) +
  scale_y_continuous(limits = c(0,30), name = "Height (cm)")+
  theme_bw() + 
...
```{r}
agricolae::kruskal(crecimiento$Height,crecimiento$Specie, p.adj = "fdr", group = TRUE, console = TRUE)

Pch <- crecimiento %>% filter(Specie == "P. chilensis")
agricolae::kruskal(Pch$Height,Pch$Soil, p.adj = "fdr", group = TRUE, console = TRUE)

Pta <- crecimiento %>% filter(Specie == "P. tamarugo")
agricolae::kruskal(Pta$Height,Pta$Soil, p.adj = "fdr", group = TRUE, console = TRUE)
```

#Water potential
```{r}
water <- read.csv("water_potential_2019.csv", dec = ".", sep = ";")

water$Specie = factor(water$Specie, levels=c('P. chilensis','P. tamarugo'))
water$Period = factor(water$Period, levels=c('Growth','Drought','Recovery'))
water$Treatment = factor(water$Treatment, levels=c('Control','Stress'))
water$Soil <- rep(c('Sand' = 'La Serena',
                          'Central' = 'Vicuña',
                          'South' = 'Los Angeles'),each = 5)
water$Soil = factor(water$Soil, levels=c('La Serena','Vicuña','Los Angeles'))
water$WP <- water$WP*-1
```
...
```{r}
#pdf("Manuscript/Plots/water_potential.pdf", width = 19, height = 19)
ggplot(water, aes(x=Specie, y=WP, fill=Soil)) +
  stat_boxplot(geom = "errorbar") +
  geom_boxplot(size = 1) + 
  scale_fill_manual(values = c("gold","darkgoldenrod3","darkorange")) +
  scale_y_continuous(name = "Water potential (MPa)") +
  theme_bw() + 
  theme(text=element_text(size = 18),
        axis.text.x = element_text(size = 16, colour = "black"),
        axis.text.y = element_text(size = 16, colour = "black"),
...
```{r}
agricolae::kruskal(water$WP,water$Specie, p.adj = "fdr", group = TRUE, console = TRUE)

Pch <- water %>% filter(Specie == "P. chilensis")
agricolae::kruskal(Pch$WP,Pch$Soil, p.adj = "fdr", group = TRUE, console = TRUE)

Pta <- water %>% filter(Specie == "P. tamarugo")
agricolae::kruskal(Pta$WP,Pta$Soil, p.adj = "fdr", group = TRUE, console = TRUE)
```

#CFU
```{r}
UFC <- read.csv("UFC_2019.csv", dec = ".", sep = ";")

UFC$Micro = factor(UFC$Micro, levels=c('Bacteria','Fungus'))
UFC$Soil <- rep(c('North' = 'Pozo Almonte',
                  'Sand' = 'La Serena',
                  'Central' = 'Vicuña',
                  'South' = 'Los Angeles'),each = 3)
UFC$Soil = factor(UFC$Soil, levels=c('Pozo Almonte','La Serena','Vicuña','Los Angeles'))
```

```{r}
point <- scales::format_format(big.mark = "", decimal.mark = ",", scientific = FALSE)

#pdf("Manuscript/Plots/CFU.pdf", width = 19, height = 19)
ggplot(UFC) + 
  aes(x=Soil, y=CFU, fill =Soil) +
  stat_boxplot(geom = "errorbar") +
  geom_boxplot(size = 1) + 
  ...
  theme(text=element_text(size = 18),
        ...
        axis.title.x = element_blank(),
        panel.border = element_rect(size = 2, fill = NA)) +
  scale_y_continuous(name = "Colony-Forming Units", labels = point) +
...
```{r}
#bacterial CFU
aggregate(UFC[1:12,3], list(UFC[1:12,1]), median)
kruskal.test(CFU ~ Soil, data = UFC[1:12,])
tmp <- dunnTest(CFU ~ Soil, data = UFC[1:12,], method = "bonferroni")
tmp <- tmp$res
cldList(comparison = tmp$Comparison, p.value = tmp$P.unadj)

summary(aov(CFU ~ Soil, data = UFC[1:12,]))
TukeyHSD(aov(CFU ~ Soil, data = UFC[1:12,]))
agricolae::HSD.test(aov(CFU~Soil, data = UFC[1:12,]), trt = 'Soil', group = T, console = T)

agricolae::kruskal(UFC[1:12,]$CFU, UFC[1:12,]$Soil, p.adj = "bonferroni", group = T, console = T)

#fungal CFU
aggregate(UFC[13:24,3], list(UFC[13:24,1]), median)
kruskal.test(CFU ~ Soil, data = UFC[13:24,])
tmp <- dunnTest(CFU ~ Soil, data = UFC[13:24,], method = "bonferroni")
tmp <- tmp$res
cldList(comparison = tmp$Comparison, p.value = tmp$P.adj)

agricolae::kruskal(UFC[13:24,]$CFU, UFC[13:24,]$Soil, p.adj = "holm", group = T, console = T)
...
```{r}
# ASV table
ASVs <- read.table("Amplicon/ITS2/table_ITS2.txt", sep="\t", header=TRUE, row.names=1)
# taxonomy
taxonomy <- read.table("Amplicon/ITS2/taxonomy.tsv", sep="\t", header=TRUE, row.names=1, na.strings=c(""," ","NA"))
taxonomy <- as.matrix(taxonomy)
# tree
tree <- read_tree("Amplicon/ITS2/tree.nwk")
# sample data
metadata <- read.table("Amplicon/ITS2/metadata.tsv", sep="\t", header=TRUE, row.names=1, na.strings=c(""," ","NA"),dec = ",")
# phyloseq object
...
```{r}
#Roots
#pdf("Manuscript/Plots/Supplementary/Fungal_Venn_Pch_root.pdf")
plot.new()
...
#Soil
#pdf("Manuscript/Plots/Supplementary/Fungal_Venn_Pch_Soil.pdf")
plot.new()
suppressWarnings(grid.draw(venn.diagram(
...
```{r}
#Roots
#pdf("Manuscript/Plots/Supplementary/Fungal_Venn_Pta_root.pdf")
plot.new()
suppressWarnings(grid.draw(venn.diagram(
...
#Soils
#pdf("Manuscript/Plots/Supplementary/Fungal_Venn_Pta_soil.pdf")
plot.new()
suppressWarnings(grid.draw(venn.diagram(
...
Plotting richness
```{r echo= FALSE}
#pdf("Manuscript/Plots/Fungal_Richness_soil.pdf")
...
```{r echo= FALSE}
#pdf("Manuscript/Plots/Fungal_Richness.pdf")
...
```{r echo= FALSE}
#pdf("Manuscript/Plots/Fungal_alpha_soil.pdf")
...
```{r echo= FALSE}
#pdf("Manuscript/Plots/Fungal_alpha.pdf")
...
### Evenness
```{r}
even <- otu_table(phy_obj_Fungi)
even <- as.data.frame(even)

tax <- paste(tax_table(phy_obj_Fungi)[,1],tax_table(phy_obj_Fungi)[,2],tax_table(phy_obj_Fungi)[,3],
      tax_table(phy_obj_Fungi)[,4],tax_table(phy_obj_Fungi)[,5],tax_table(phy_obj_Fungi)[,6],
      tax_table(phy_obj_Fungi)[,7], sep = "; ")
even[,"taxonomy"] <- tax

even <- RAM::evenness(data=list(even=even), index = "shannon")
even <- as.data.frame(t(even))

even <- merge(even,sample_data(phy_obj_Fungi), by = "row.names")
even$Soil <- factor(even$Soil, levels=c('Pozo_Almonte','La_Serena','Vicuna','Los_Angeles'))
```

```{r}
#pdf("Manuscript/Plots/Fungal_even_soil.pdf")
...
```{r}
#pdf("Manuscript/Plots/Fungal_even.pdf")
ggplot(even, aes(x = Specie, y = even, fill = Soil)) +
  ...
  theme(text=element_text(size = 18),
        ...
        axis.title.x = element_blank(),
        panel.border = element_rect(size = 2, fill = NA)) +
  scale_y_continuous(name = "Pielou's Evenness",limits = c(0,1)) + 
  facet_grid(.~Sample)
#dev.off()
```
...
#write.csv(df,"Manuscript/Plots/Supplementary/Fungal_soil_div.csv")
```
...
#write.csv(df,"Manuscript/Plots/Supplementary/Fungal_richness_alpha.csv")
```
...
#pdf("Manuscript/Plots/Fungal_beta_soil.pdf",height=10, width=10)
...
#pdf("Manuscript/Plots/Fungal_beta_root.pdf",height=10, width=10)
plot_ordination(phy_norm, ord, color = "Soil", shape = "Specie_sample") + geom_point(size=7) +
...
#pdf("Manuscript/Plots/Fungal_beta.pdf",height=10, width=10)
plot_ordination(phy_norm, ord, color = "Soil", shape = "Specie_sample") + geom_point(size=7) +
...
#pdf("Manuscript/Plots/Fungal_beta_2.pdf",height=10, width=10)
...
```{r}
sample_data(phy_obj_Fungi)[,"samples_sp"] <- paste(sample_data(phy_obj_Fungi)$Sample,sample_data(phy_obj_Fungi)$Specie,sample_data(phy_obj_Fungi)$Soil, sep = ".")
...
#' Merging data
phy_mrg <- merge_samples(phy_obj_Fungi, group = "samples_sp", fun = "mean")
...
Fungal_cols <- scheme$hex(n = 10, force_mode = FALSE, quality = 50, color_space = col_hcl)
```
...
```{r}
#pdf("Manuscript/Plots/Supplementary/fungal_phyla_Pch_root.pdf", height = 10, width = 15)
ggplot(m, aes(y= Percentage, x = Sample, fill = Phylum))+
  ...
  scale_fill_manual(values = Fungal_cols) + 
  coord_flip()
#dev.off()
```
...
```{r}
#pdf("Manuscript/Plots/Supplementary/fungal_phyla_Pch_soil.pdf", height = 10, width = 15)
ggplot(m, aes(y= Percentage, x = Sample, fill = Phylum))+
  geom_bar(stat = "identity", position = "stack", width = 0.5)+
  geom_text(data=m, aes(x = Sample,y= Percentage, label = paste0(Percentage,"%")),
            position = position_stack(vjust = .5),
...
```{r}
#pdf("Manuscript/Plots/Supplementary/fungal_phyla_Pta_root.pdf", height = 10, width = 15)
ggplot(m, aes(y= Percentage, x = Sample, fill = Phylum))+
  geom_bar(stat = "identity", position = "stack", width = 0.5)+
  geom_text(data=m, aes(x = Sample,y= Percentage, label = paste0(Percentage,"%")),
            position = position_stack(vjust = .5),
...
```{r}
#pdf("Manuscript/Plots/Supplementary/fungal_phyla_Pta_soil.pdf", height = 10, width = 15)
ggplot(m, aes(y= Percentage, x = Sample, fill = Phylum))+
  geom_bar(stat = "identity", position = "stack", width = 0.5)+
  geom_text(data=m, aes(x = Sample,y= Percentage, label = paste0(Percentage,"%")),
            position = position_stack(vjust = .5),
...
``` {r echo= FALSE}
#' Merging data
#phy_asco <- subset_taxa(phy_obj_Fungi, Phylum == "Ascomycota")

#phy_obj2 <- tax_glom(phy_asco, taxrank = "Family", NArm = FALSE)

#phy_mrg <- merge_samples(phy_obj2, group = "samples_sp", fun = "mean")
...
pheatmap(phy_mrg[,c(1,3,2,9,7,10,8)], col=colors, breaks=bk,
         ...
         main = "100 most abundant fungal species \n P. chilensis", 
         labels_row = paste(phy_mrg$taxonomy,rownames(phy_mrg),sep = ",")
         , annotation_legend = FALSE
         , show_rownames = FALSE
         , na_col = 'gray'
         #, cellheight = 8
         #, filename = "Manuscript/Plots/Supplementary/dominant_families_fungi_Pc.pdf", height = 45, width = 30
)
#dev.off()
```
...
pheatmap(phy_mrg[,c(4,6,5,13,11,14,12)], col=colors, breaks=bk,
         ...
         , na_col = 'gray'
         #, cellheight = 8
         #, filename = "Manuscript/Plots/Supplementary/dominant_families_fungi_Pt.pdf", height = 45, width = 30
)
#dev.off()
```
...
pheatmap(phy_mrg[,c(1,3,2,4,6,5,9,7,10,8,13,11,14,12)], col=colors, breaks=bk,
         angle_col = 45, cellwidth = 12,
         cluster_cols = FALSE, cluster_rows = FALSE, annotation_col = col_ann, 
         annotation_colors = pal, fontsize_col=8, fontsize_row=8, gaps_col = c(3,6,10),
         main = "100 most abundant fungal species", 
         labels_row = paste(phy_mrg$taxonomy,rownames(phy_mrg),sep = ",")
         , annotation_legend = FALSE
         , show_rownames = FALSE
         , na_col = 'gray'
         #, cellheight = 8
         #, filename = "Manuscript/Plots/Supplementary/100_dominant_fungi.pdf", height = 45, width = 30
)
#dev.off()
```
...
```{r}
sample_data(phy_obj_Fungi)[,"Sample_type"] <- paste(sample_data(phy_obj_Fungi)$Specie,sample_data(phy_obj_Fungi)$Soil,sample_data(phy_obj_Fungi)$Sample,sep = "_")
diagdds = phyloseq_to_deseq2(phy_obj_Fungi, ~ Sample_type)
...
sigtab_PA = cbind(as(resP_Al, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_Al), ], "matrix"))
...
#pdf(file = "Manuscript/Plots/Fungi_Pal.pdf", height = 15, width = 15)
...
sigtab_Pch = cbind(as(resP_ch_V, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_V), ], "matrix"))
...
sigtab_Pta = cbind(as(resP_ta_V, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_V), ], "matrix"))
...
#pdf(file = "Manuscript/Plots/Fungi_Vic.pdf", height = 15, width = 15)
...
sigtab_Pch = cbind(as(resP_ch_LS, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_LS), ], "matrix"))
sigtab_Pch <- subset(sigtab_Pch, padj <= 0.01 & abs(log2FoldChange) >= 0.5)
sigtab_Pch$ASV = rownames(sigtab_Pch)

resP_ta_LS <- results(diagdds, contrast = c("Sample_type", "P_tamarugo_La_Serena_Root", "P_tamarugo_La_Serena_Soil"))
sigtab_Pta = cbind(as(resP_ta_LS, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_LS), ], "matrix"))
sigtab_Pta <- subset(sigtab_Pta, padj <= 0.01 & abs(log2FoldChange) >= 0.5)
sigtab_Pta$ASV = rownames(sigtab_Pta)
...
#pdf(file = "Manuscript/Plots/Fungi_LSe.pdf", height = 15, width = 15)
...
sigtab_Pch = cbind(as(resP_ch_LA, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_LA), ], "matrix"))
sigtab_Pch <- subset(sigtab_Pch, padj <= 0.01 & abs(log2FoldChange) >= 0.5)
sigtab_Pch$ASV = rownames(sigtab_Pch)

resP_ta_LA <- results(diagdds, contrast = c("Sample_type", "P_tamarugo_Los_Angeles_Root", "P_tamarugo_Los_Angeles_Soil"))
sigtab_Pta = cbind(as(resP_ta_LA, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_LA), ], "matrix"))
sigtab_Pta <- subset(sigtab_Pta, padj <= 0.01 & abs(log2FoldChange) >= 0.5)
sigtab_Pta$ASV = rownames(sigtab_Pta)
...
#pdf(file = "Manuscript/Plots/Fungi_LAn.pdf", height = 15, width = 15)
...
#pdf(file = "Manuscript/Plots/Fungi_all.pdf", height = 15, width = 15)
...
#P_chilensis
#soil vs soil
sigtabP_ch_S1 = cbind(as(resP_ch_S1, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_S1), ], "matrix"))
sigtabP_ch_S2 = cbind(as(resP_ch_S2, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_S2), ], "matrix"))
sigtabP_ch_S3 = cbind(as(resP_ch_S3, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_S3), ], "matrix"))
sigtabP_ch_S4 = cbind(as(resP_ch_S4, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_S4), ], "matrix"))
sigtabP_ch_S5 = cbind(as(resP_ch_S5, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_S5), ], "matrix"))
sigtabP_ch_S6 = cbind(as(resP_ch_S6, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_S6), ], "matrix"))

#root vs root
sigtabP_ch_R1 = cbind(as(resP_ch_R1, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_R1), ], "matrix"))
sigtabP_ch_R2 = cbind(as(resP_ch_R2, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_R2), ], "matrix"))
sigtabP_ch_R3 = cbind(as(resP_ch_R3, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_R3), ], "matrix"))

#soil vs root
sigtabP_ch_SR1 = cbind(as(resP_ch_SR1, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR1), ], "matrix"))
sigtabP_ch_SR2 = cbind(as(resP_ch_SR2, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR2), ], "matrix"))
sigtabP_ch_SR3 = cbind(as(resP_ch_SR3, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR3), ], "matrix"))
sigtabP_ch_SR4 = cbind(as(resP_ch_SR4, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR4), ], "matrix"))
sigtabP_ch_SR5 = cbind(as(resP_ch_SR5, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR5), ], "matrix"))
sigtabP_ch_SR6 = cbind(as(resP_ch_SR6, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR6), ], "matrix"))
sigtabP_ch_SR7 = cbind(as(resP_ch_SR7, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR7), ], "matrix"))
sigtabP_ch_SR8 = cbind(as(resP_ch_SR8, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR8), ], "matrix"))
sigtabP_ch_SR9 = cbind(as(resP_ch_SR9, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR9), ], "matrix"))
sigtabP_ch_SR10 = cbind(as(resP_ch_SR10, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR10), ], "matrix"))
sigtabP_ch_SR11 = cbind(as(resP_ch_SR11, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR11), ], "matrix"))
sigtabP_ch_SR12 = cbind(as(resP_ch_SR12, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ch_SR12), ], "matrix"))

#P_tamarugo
#soil vs soil
sigtabP_ta_S1 = cbind(as(resP_ta_S1, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_S1), ], "matrix"))
sigtabP_ta_S2 = cbind(as(resP_ta_S2, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_S2), ], "matrix"))
sigtabP_ta_S3 = cbind(as(resP_ta_S3, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_S3), ], "matrix"))
sigtabP_ta_S4 = cbind(as(resP_ta_S4, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_S4), ], "matrix"))
sigtabP_ta_S5 = cbind(as(resP_ta_S5, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_S5), ], "matrix"))
sigtabP_ta_S6 = cbind(as(resP_ta_S6, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_S6), ], "matrix"))

#root vs root
sigtabP_ta_R1 = cbind(as(resP_ta_R1, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_R1), ], "matrix"))
sigtabP_ta_R2 = cbind(as(resP_ta_R2, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_R2), ], "matrix"))
sigtabP_ta_R3 = cbind(as(resP_ta_R3, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_R3), ], "matrix"))

#soil vs root
sigtabP_ta_SR1 = cbind(as(resP_ta_SR1, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR1), ], "matrix"))
sigtabP_ta_SR2 = cbind(as(resP_ta_SR2, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR2), ], "matrix"))
sigtabP_ta_SR3 = cbind(as(resP_ta_SR3, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR3), ], "matrix"))
sigtabP_ta_SR4 = cbind(as(resP_ta_SR4, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR4), ], "matrix"))
sigtabP_ta_SR5 = cbind(as(resP_ta_SR5, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR5), ], "matrix"))
sigtabP_ta_SR6 = cbind(as(resP_ta_SR6, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR6), ], "matrix"))
sigtabP_ta_SR7 = cbind(as(resP_ta_SR7, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR7), ], "matrix"))
sigtabP_ta_SR8 = cbind(as(resP_ta_SR8, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR8), ], "matrix"))
sigtabP_ta_SR9 = cbind(as(resP_ta_SR9, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR9), ], "matrix"))
sigtabP_ta_SR10 = cbind(as(resP_ta_SR10, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR10), ], "matrix"))
sigtabP_ta_SR11 = cbind(as(resP_ta_SR11, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR11), ], "matrix"))
sigtabP_ta_SR12 = cbind(as(resP_ta_SR12, "data.frame"), as(tax_table(phy_obj_Fungi)[rownames(resP_ta_SR12), ], "matrix"))
```
...
bk = c(seq(0.0,0.584,length=240),seq(0.585,1.168,length=65),seq(1.169,1.752,length=65),seq(1.753,2.336,length=65),seq(2.337,2.92,length=65))
...
```{r}
#pdf(file = "Manuscript/Plots/Deferentially_abundant_fungi_Pch.pdf", height = 15, width = 15)
...
```{r}
#pdf(file = "Manuscript/Plots/Differentially_abundant_fungi_Pta.pdf", height = 15, width = 15)
...
phy_diff_Fungi <- as.data.frame(t(otu_table(phy_diff)))

phy_diff_Fungi[,"ASV"] <- rownames(phy_diff_Fungi)
tax[,"ASV"] <- rownames(tax)
phy_diff_Fungi <- join(phy_diff_Fungi, tax, by= "ASV", type = "left")
rownames(phy_diff_Fungi) <- phy_diff_Fungi$ASV

hpal <- colorRampPalette(RColorBrewer::brewer.pal(9, "Reds"))(100)

phy_diff_Fungi <- mutate(phy_diff_Fungi, taxo = paste(Kingdom, Phylum, Class, Order, Family, Genus, Specie, ASV, sep = "; ")) 
```

```{r}
#pdf(file = "Manuscript/Plots/Differentially_abundant_fungi_both.pdf", height = 15, width = 15)
heatmap.2(as.matrix(phy_diff_Fungi[,1:14]),
          Colv=FALSE, Rowv = FALSE, dendrogram = 'none', col = hpal,
          cexRow = .2, cexCol = .5, offsetRow = -1, 
          tracecol = 'black', labRow = phy_diff_Fungi$taxo, margin=c(2, 8), 
          offsetCol=0, srtCol=0, adjCol = c(0.5,1),  
          main="Differentially abundant Fungi \n Both")
...
## Correlation plot
```{r}
corr_fungi <- faiths_div %>%
  dplyr::select(Row.names, SR, Sample) %>%
  filter(Sample=="Soil") %>%
  left_join(
    shannon_div, by = "Row.names")

#Hmisc::rcorr(as.matrix(corr_fungi_2),type="spearman")
suppressWarnings(car::scatterplotMatrix(as.matrix(corr_fungi %>% 
  dplyr::select(SR, shannon_div, Chlorides, Phosphates, Boron, Sulphates, Carbonates, Calcium, Magnesium, Sodium)),regLine = list(method=lm,col="red")))

suppressWarnings(car::scatterplotMatrix(as.matrix(corr_fungi %>% 
  dplyr::select(SR, shannon_div,Potassium, Lithium, Copper, Iron, Manganese, Zinc, Lead)),regLine = list(method=lm,col="red")))

#Hmisc::rcorr(as.matrix(corr_fungi_3),type="spearman")
suppressWarnings(car::scatterplotMatrix(as.matrix(corr_fungi %>% dplyr::select(SR, shannon_div, Field_capacity, CFU, Organic_matter,  pH, Conductivity)),regLine = list(method=lm,col="red")))
...
#Hmisc::rcorr(as.matrix(corr_fungi_2),type="spearman")
suppressWarnings(car::scatterplotMatrix(as.matrix(corr_fungi %>% 
  dplyr::select(SR, shannon_div, Shoot_height, Shoot_Water_potetial)),regLine = list(method=lm,col="red")))
```

### Prosopis chilensis
#### Root
```{r}
phy_Pch <- subset_samples(phy_obj_Fungi, Specie == "P_chilensis")
phy_Pch <- subset_samples(phy_Pch, Sample == "Root")

norm_phy_obj_Pch_root_Fungi <- otu_table(phy_Pch)[rownames(otu_table(phy_Pch)) %in% phy_diff_Pch$ASV,]
col_order <- sort(colnames(norm_phy_obj_Pch_root_Fungi),decreasing = FALSE)
norm_phy_obj_Pch_root_Fungi <- norm_phy_obj_Pch_root_Fungi[,col_order]
norm_phy_obj_Pch_root_Fungi <- as.data.frame(norm_phy_obj_Pch_root_Fungi)
barplot(colSums(norm_phy_obj_Pch_root_Fungi))
norm_phy_obj_Pch_root_Fungi <- clusterSim::data.Normalization(norm_phy_obj_Pch_root_Fungi, type = "n10", normalization = "column")
barplot(colSums(norm_phy_obj_Pch_root_Fungi))
norm_phy_obj_Pch_root_Fungi[is.na(norm_phy_obj_Pch_root_Fungi)] <- 0

norm_metadata <- metadata[rownames(metadata) %in% colnames(norm_phy_obj_Pch_root_Fungi),]
...
phy_obj_metadata_corr = cor(t(norm_phy_obj_Pch_root_Fungi), norm_metadata, method = "spearman")
...
```{r}
#pdf("Manuscript/Plots/Correlation_Root_Fungi_Pch.pdf", height = 14, width = 15)
...
norm_phy_obj_Pch_soil_Fungi <- otu_table(phy_Pch)[rownames(otu_table(phy_Pch)) %in% phy_diff_Pch$ASV,]
col_order <- sort(colnames(norm_phy_obj_Pch_soil_Fungi),decreasing = FALSE)
norm_phy_obj_Pch_soil_Fungi <- norm_phy_obj_Pch_soil_Fungi[,col_order]
norm_phy_obj_Pch_soil_Fungi <- as.data.frame(norm_phy_obj_Pch_soil_Fungi)
barplot(colSums(norm_phy_obj_Pch_soil_Fungi))
norm_phy_obj_Pch_soil_Fungi <- clusterSim::data.Normalization(norm_phy_obj_Pch_soil_Fungi, type = "n10", normalization = "column")
barplot(colSums(norm_phy_obj_Pch_soil_Fungi))
norm_phy_obj_Pch_soil_Fungi[is.na(norm_phy_obj_Pch_soil_Fungi)] <- 0

norm_metadata <- metadata[rownames(metadata) %in% colnames(norm_phy_obj_Pch_soil_Fungi),]
...
phy_obj_metadata_corr = cor(t(norm_phy_obj_Pch_soil_Fungi), norm_metadata, method = "spearman")
phy_obj_metadata_corr <- as.data.frame(phy_obj_metadata_corr) %>% drop_na()
...
```{r}
#pdf("Manuscript/Plots/Correlation_soil_Fungi_Pch.pdf", height = 14, width = 15)
heatmap.obj.tax.path.top = Heatmap(phy_obj_metadata_corr,
                                   name = "Comparison Top Taxa to Top Pathways",
                                   col=colorRamp2(c(-1,0,1),c("darkblue","white","darkred")),
                                   show_row_dend = FALSE,
...
norm_phy_obj_Pch_Fungi <- otu_table(phy_Pch)[rownames(otu_table(phy_Pch)) %in% phy_diff_Pch$ASV,]
col_order <- sort(colnames(norm_phy_obj_Pch_Fungi),decreasing = FALSE)
norm_phy_obj_Pch_Fungi <- norm_phy_obj_Pch_Fungi[,col_order]
norm_phy_obj_Pch_Fungi <- as.data.frame(norm_phy_obj_Pch_Fungi)
barplot(colSums(norm_phy_obj_Pch_Fungi))
norm_phy_obj_Pch_Fungi <- clusterSim::data.Normalization(norm_phy_obj_Pch_Fungi, type = "n10", normalization = "column")
barplot(colSums(norm_phy_obj_Pch_Fungi))
norm_phy_obj_Pch_Fungi[is.na(norm_phy_obj_Pch_Fungi)] <- 0

norm_metadata <- metadata[rownames(metadata) %in% colnames(norm_phy_obj_Pch_Fungi),]
...
phy_obj_metadata_corr = cor(t(norm_phy_obj_Pch_Fungi), norm_metadata, method = "spearman")
phy_obj_metadata_corr <- as.data.frame(phy_obj_metadata_corr) %>% drop_na()
...
```{r}
#pdf("Manuscript/Plots/Correlation_Fungi_Pch.pdf", height = 14, width = 15)
heatmap.obj.tax.path.top = Heatmap(phy_obj_metadata_corr,
                                   name = "Comparison Top Taxa to Top Pathways",
                                   col=colorRamp2(c(-1,0,1),c("darkblue","white","darkred")),
                                   show_row_dend = FALSE,
...
norm_phy_obj_Pta_root_Fungi <- otu_table(phy_Pta)[rownames(otu_table(phy_Pta)) %in% phy_diff_Pta$ASV,]
col_order <- sort(colnames(norm_phy_obj_Pta_root_Fungi),decreasing = FALSE)
norm_phy_obj_Pta_root_Fungi <- norm_phy_obj_Pta_root_Fungi[,col_order]
norm_phy_obj_Pta_root_Fungi <- as.data.frame(norm_phy_obj_Pta_root_Fungi)
barplot(colSums(norm_phy_obj_Pta_root_Fungi))
norm_phy_obj_Pta_root_Fungi <- clusterSim::data.Normalization(norm_phy_obj_Pta_root_Fungi, type = "n10", normalization = "column")
barplot(colSums(norm_phy_obj_Pta_root_Fungi))
norm_phy_obj_Pta_root_Fungi[is.na(norm_phy_obj_Pta_root_Fungi)] <- 0

norm_metadata <- metadata[rownames(metadata) %in% colnames(norm_phy_obj_Pta_root_Fungi),]
norm_metadata <- norm_metadata[,8:9]
barplot(rowSums(norm_metadata))
norm_metadata <- clusterSim::data.Normalization(norm_metadata, type = "n10", normalization = "row")
barplot(rowSums(norm_metadata))
...
phy_obj_metadata_corr = cor(t(norm_phy_obj_Pta_root_Fungi), norm_metadata, method = "spearman")
phy_obj_metadata_corr <- as.data.frame(phy_obj_metadata_corr) %>% drop_na()
...
```{r}
#pdf("Manuscript/Plots/Correlation_Root_Fungi_Pta.pdf", height = 14, width = 15)
heatmap.obj.tax.path.top = Heatmap(phy_obj_metadata_corr,
                                   name = "Comparison Top Taxa to Top Pathways",
                                   col=colorRamp2(c(-1,0,1),c("darkblue","white","darkred")),
                                   show_row_dend = FALSE,
...
norm_phy_obj_Pta_soil_Fungi <- otu_table(phy_Pta)[rownames(otu_table(phy_Pta)) %in% phy_diff_Pta$ASV,]
col_order <- sort(colnames(norm_phy_obj_Pta_soil_Fungi),decreasing = FALSE)
norm_phy_obj_Pta_soil_Fungi <- norm_phy_obj_Pta_soil_Fungi[,col_order]
norm_phy_obj_Pta_soil_Fungi <- as.data.frame(norm_phy_obj_Pta_soil_Fungi)
barplot(colSums(norm_phy_obj_Pta_soil_Fungi))
norm_phy_obj_Pta_soil_Fungi <- clusterSim::data.Normalization(norm_phy_obj_Pta_soil_Fungi, type = "n10", normalization = "column")
barplot(colSums(norm_phy_obj_Pta_soil_Fungi))
norm_phy_obj_Pta_soil_Fungi[is.na(norm_phy_obj_Pta_soil_Fungi)] <- 0

norm_metadata <- metadata[rownames(metadata) %in% colnames(norm_phy_obj_Pta_soil_Fungi),]
norm_metadata <- norm_metadata[,10:29]
barplot(rowSums(norm_metadata))
norm_metadata <- clusterSim::data.Normalization(norm_metadata, type = "n10", normalization = "row")
barplot(rowSums(norm_metadata))
...
phy_obj_metadata_corr = cor(t(norm_phy_obj_Pta_soil_Fungi), norm_metadata, method = "spearman")
phy_obj_metadata_corr <- as.data.frame(phy_obj_metadata_corr) %>% drop_na()
...
```{r}
#pdf("Manuscript/Plots/Correlation_soil_Fungi_Pta.pdf", height = 14, width = 15)
heatmap.obj.tax.path.top = Heatmap(phy_obj_metadata_corr,
                                   name = "Comparison Top Taxa to Top Pathways",
                                   col=colorRamp2(c(-1,0,1),c("darkblue","white","darkred")),
                                   show_row_dend = FALSE,
...
norm_phy_obj_Pta_Fungi <- otu_table(phy_Pta)[rownames(otu_table(phy_Pta)) %in% phy_diff_Pta$ASV,]
col_order <- sort(colnames(norm_phy_obj_Pta_Fungi),decreasing = FALSE)
norm_phy_obj_Pta_Fungi <- norm_phy_obj_Pta_Fungi[,col_order]
norm_phy_obj_Pta_Fungi <- as.data.frame(norm_phy_obj_Pta_Fungi)
barplot(colSums(norm_phy_obj_Pta_Fungi))
norm_phy_obj_Pta_Fungi <- clusterSim::data.Normalization(norm_phy_obj_Pta_Fungi, type = "n10", normalization = "column")
barplot(colSums(norm_phy_obj_Pta_Fungi))
norm_phy_obj_Pta_Fungi[is.na(norm_phy_obj_Pta_Fungi)] <- 0

norm_metadata <- metadata[rownames(metadata) %in% colnames(norm_phy_obj_Pta_Fungi),]
norm_metadata <- norm_metadata[,8:29]
norm_metadata[is.na(norm_metadata)] <- 0
barplot(rowSums(norm_metadata))
norm_metadata <- clusterSim::data.Normalization(norm_metadata, type = "n10", normalization = "row")
...
phy_obj_metadata_corr = cor(t(norm_phy_obj_Pta_Fungi), norm_metadata, method = "spearman")
phy_obj_metadata_corr <- as.data.frame(phy_obj_metadata_corr) %>% drop_na()
...
```{r}
#pdf("Manuscript/Plots/Correlation_Fungi_Pta.pdf", height = 14, width = 15)
heatmap.obj.tax.path.top = Heatmap(phy_obj_metadata_corr,
                                   name = "Comparison Top Taxa to Top Pathways",
                                   col=colorRamp2(c(-1,0,1),c("darkblue","white","darkred")),
                                   show_row_dend = FALSE,
...
### Both
```{r}
norm_phy_obj_Fungi <- otu_table(phy_obj_Fungi)[rownames(otu_table(phy_obj_Fungi)) %in% phy_diff_Fungi$ASV,]
col_order <- sort(colnames(norm_phy_obj_Fungi),decreasing = FALSE)
norm_phy_obj_Fungi <- norm_phy_obj_Fungi[,col_order]
norm_phy_obj_Fungi <- as.data.frame(norm_phy_obj_Fungi)
barplot(colSums(norm_phy_obj_Fungi))
norm_phy_obj_Fungi <- clusterSim::data.Normalization(norm_phy_obj_Fungi, type = "n10", normalization = "column")
barplot(colSums(norm_phy_obj_Fungi))
norm_phy_obj_Fungi[is.na(norm_phy_obj_Fungi)] <- 0

colnames(norm_phy_obj_Fungi)
rownames(norm_metadata)

norm_metadata <- sample_data(phy_obj_Fungi)[rownames(sample_data(phy_obj_Fungi)) %in% colnames(norm_phy_obj_Fungi),]
norm_metadata <- norm_metadata[,8:29]
norm_metadata[is.na(norm_metadata)] <- 0
barplot(rowSums(norm_metadata))
norm_metadata <- clusterSim::data.Normalization(norm_metadata, type = "n10", normalization = "row")
...
phy_obj_metadata_corr = cor(t(norm_phy_obj_Fungi[order(rowSums(norm_phy_obj_Fungi),decreasing = TRUE),])[,1:100], norm_metadata, method = "spearman")

phy_obj_metadata_corr <- na.omit(phy_obj_metadata_corr)

rownames(phy_obj_metadata_corr) <- paste(tax_table(phy_obj_Fungi)[,1][rownames(otu_table(phy_obj_Fungi)) %in% rownames(phy_obj_metadata_corr),],                                      tax_table(phy_obj_Fungi)[,2][rownames(otu_table(phy_obj_Fungi)) %in% rownames(phy_obj_metadata_corr),],                                       tax_table(phy_obj_Fungi)[,3][rownames(otu_table(phy_obj_Fungi)) %in% rownames(phy_obj_metadata_corr),],                                       tax_table(phy_obj_Fungi)[,4][rownames(otu_table(phy_obj_Fungi)) %in% rownames(phy_obj_metadata_corr),],                                       tax_table(phy_obj_Fungi)[,5][rownames(otu_table(phy_obj_Fungi)) %in% rownames(phy_obj_metadata_corr),],                                        tax_table(phy_obj_Fungi)[,6][rownames(otu_table(phy_obj_Fungi)) %in% rownames(phy_obj_metadata_corr),],                                        tax_table(phy_obj_Fungi)[,7][rownames(otu_table(phy_obj_Fungi)) %in% rownames(phy_obj_metadata_corr),], rownames(tax_table(phy_obj_Fungi))[rownames(otu_table(phy_obj_Fungi)) %in% rownames(phy_obj_metadata_corr)], sep = ", ")
```

```{r}
#pdf("Manuscript/Plots/Correlation_Fungi.pdf", height = 14, width = 15)
heatmap.obj.tax.path.top = Heatmap(phy_obj_metadata_corr,
                                   name = "Comparison Top Taxa to Top Pathways",
                                   col=colorRamp2(c(-1,0,1),c("darkblue","white","darkred")),
                                   show_row_dend = FALSE,
...
```{r}
# ASV table
ASVs <- read.table("Amplicon/16S/table_16S.txt", sep="\t", header=TRUE, row.names=1)
# taxonomy
taxonomy <- read.table("Amplicon/16S/taxonomy.tsv", sep="\t", header=TRUE, row.names=1, na.strings=c(""," ","NA"))
taxonomy <- as.matrix(taxonomy)
# tree
tree <- read_tree("Amplicon/16S/tree.nwk")
# sample data
metadata <- read.table("Amplicon/16S/metadata.tsv", sep="\t", header=TRUE, row.names=1, na.strings=c(""," ","NA"),dec = ",")
# phyloseq object
phy_obj <- phyloseq(otu_table(ASVs, taxa_are_rows = TRUE), 
                    tax_table(taxonomy), sample_data(metadata),tree)
